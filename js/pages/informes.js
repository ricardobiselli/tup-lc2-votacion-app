...
async function pantallaInicio() {
  if (informesGuardadosStorage && informesGuardadosStorage.length > 0) {
    ...
    try {
      for (const informe of informesGuardadosStorage) {
        const tbody = document.getElementById("contenedor-informes");

        const response = await fetch(
          `https://resultados.mininterior.gob.ar/api/resultado/totalizado?año=${informe.vAnio}&recuento=Provisorio&idEleccion=${informe.vTipoEleccion}&idCargo=${informe.vCategoriaId}&idDistrito=${informe.vDistrito}&idSeccionProvincial=${informe.vSeccionProvincial}&idSeccion=${informe.vSeccionID}&circuitoId=""&mesaId=""`
        );
        ...
        if (response.ok) {
          ...
          tdEleccionTexto.append(pTextoEleccionesChico);
          tdEleccionTexto.append(pTextoPathChico);

          const tdDatosGenerales = document.createElement("td");
          tdDatosGenerales.innerHTML = `<div class="seccion-botones">
...
<svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
    ...
    <div>
      ...
      <p id="mesas-escrutadas">${data.MesasEscrutadas}</p>
    </div>
  </div>
...
<svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
    ...
    <div>
      <p>Electores</p>
      <p id="electores">${data.Votantes}</p>
    </div>
  </div>
  <div class="icon-button participacion">
    ...
    <div>
      ...
      <p id="participacion">${data.ParticipacionSobreEscrutado}%</p>
    </div>
  </div>
</div>`;
          fila.append(tdDatosGenerales);

          const tdAgrupaciones = document.createElement("td");
          tdAgrupaciones.classList.add("agrupaciones");
          fila.append(tdAgrupaciones);
          ...
          data.agrupaciones.forEach((agrupacion) => {
            ...
            tdAgrupaciones.append(agrupacionTexto);

            const h3 = document.createElement("h3");
            h3.innerText = agrupacion.nombre;
            h3.style.width = "75px";

            agrupacionTexto.append(h3);

            const div = document.createElement("div");
            agrupacionTexto.append(div);

            const votosPorcentaje = document.createElement("p");
            votosPorcentaje.innerText = `${agrupacion.porcentaje}%`;
          ...
          cartelRojo.innerHTML = `<p><i class="fa-solid fa-triangle-exclamation"></i>Error al ejecutar la solicitud: estado: ${response.status}</p>`;
    ...
    cartelAmarillo.innerHTML = `<p><i class="fa-solid fa-exclamation"></i> No hay informes guardados para mostrar</p>`;
